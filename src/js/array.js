...
function handleEventTextarea() {
    textArea.onkeyup = () => {
        let getValue = textArea.value.replace('}]', "").replace(/"},/g, "").split('{"');
        let arrComic = JSON.parse(textArea.value);
...
function handleComicApi() {
    ...
    arrComicNew.oninput = () => {
        postBtn.onclick = () => {
            if (arrComicNew.value.length >= 100) {
                ...
                alert('Chuỗi bạn nhập phải có ít nhất 100 ký tự!')
            }
        }
    }
} handleComicApi();
